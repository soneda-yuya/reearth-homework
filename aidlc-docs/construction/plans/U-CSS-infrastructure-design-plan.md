...
- **Q1 [A]**: Job リソース制限 = 現状維持（`cpu = 1` / `memory = 256Mi`）
- **Q2 [A]**: Task Timeout = 現状維持（`120s`）
- **Q3 [A]**: Max Retries = **`0`**（自動リトライ無効、手動実行前提で fail-fast と整合）
- **Q4 [A]**: Secret rotation = 手動 rotation + `version = "latest"` 追従
- **Q5 [A]**: reearth-cms ホスト前提 = **別の GCP プロジェクト / 既存の reearth-cms 環境**（本プロジェクト内で CMS 本体はデプロイしない）
- **Q6 [A]**: 監視 / アラート = ログ運用のみ（Cloud Logging 目視 + 構造化ログ / OTel Metric 実装のみ、Alerting Policy なし）
