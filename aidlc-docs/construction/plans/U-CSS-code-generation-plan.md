...
- **Q A [A]**: API 仕様の調査深度 = 最小限（Web で確認できる範囲）、不明点は Build and Test で実 API に合わせて修正
- **Q B [A]**: テストカバレッジ目標 = domain 95%+ / application 90%+ / infrastructure 70%+（全体 85%+）
- **Q C [A]**: PR 分割 = **1 PR にまとめる**（Go + Terraform + Docs、推定 +1,500 行 Go 込み）
- **Q D [A]**: SafetyMapSchema の description = MVP では最小限（Project/Model 短文、Field 空）
- **Q E [A]**: Observability 注入方針 = Constructor Injection（test しやすさ優先）
- **Q F [A]**: Retry 配置 = `cmsx.Client` 内（application 層は transient error を知らない）

回答確定済み。Phase 1〜7 を順次実装 → 1 PR で提出する。
