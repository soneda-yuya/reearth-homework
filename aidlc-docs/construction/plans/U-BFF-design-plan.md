...
- **Q1 [A]**: 認証方式 = **全 RPC で Firebase ID Token 必須**（Anonymous Auth 含む、AuthInterceptor で検証）
- **Q2 [A]**: SafetyIncident キャッシュ = **毎回 CMS 直アクセス**（キャッシュなし、MVP 規模で負荷問題なし）
- **Q3 [A]**: CrimeMap 集計 = **`crimemap/application.Aggregator` で in-memory 集計**、`color` はサーバ側で計算
- **Q4 [A]**: UserProfile 永続化 = **U-NTF と同じ Firestore `users` collection 共有**（U-BFF オーナー、U-NTF reader + ArrayRemove）
- **Q5 [A]**: FCM Token 冪等性 = **`ArrayUnion`** で追加、`device_id` はログ用のみ
- **Q6 [A]**: Connect error code = **`errs.KindOf` 一律マッピング** (ErrorInterceptor)、prod 時は `KindInternal`/`KindExternal` のメッセージマスク
- **Q7 [A]**: Observability = Span (`bff.<service>.<method>` root + 子) + 4 Metric + `app.bff.phase` 属性ログ
- **Q8 [A]**: テスト戦略 = 層別カバレッジ (domain 95 / app 90 / infra 70 / rpc 80 / interceptor 90 / 全体 85)、Connect handler e2e は `connecttest.Server`
- **Q9 [A]**: PR 分割 = **2 PR (U-ING パターン)**。PR A: Go ロジック + 全テスト (~2,500 行)、PR B: Composition Root + Terraform + Docs + CI (~500-800 行)

回答確定済み。`U-BFF-design.md`（合本版）を生成する。
